...
TEST(ClusteringSuggester, NewNamespace) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        rb.activities[generate_uuid()] = reactor_business_card_t<rdb_protocol_t>::activity_entry_t(hash_region_t<key_range_t>::universe(), reactor_business_card_t<rdb_protocol_t>::nothing_t());
    ...
    bool success = shards.add_region(hash_region_t<key_range_t>(key_range_t(key_range_t::none, store_key_t(),
                                                                            key_range_t::open, store_key_t("m"))));
    ASSERT_TRUE(success);
    success = shards.add_region(hash_region_t<key_range_t>(key_range_t(key_range_t::closed, store_key_t("m"),
                                                                       key_range_t::none, store_key_t())));
    ASSERT_TRUE(success);

    std::map<machine_id_t, int> usage;
    persistable_blueprint_t<rdb_protocol_t> blueprint
        = suggest_blueprint<rdb_protocol_t>(
            directory,
            primary_datacenter,
            affinities,
            shards,
            machine_data_centers,
            region_map_t<rdb_protocol_t, machine_id_t>(hash_region_t<key_range_t>::universe(), nil_uuid()),
